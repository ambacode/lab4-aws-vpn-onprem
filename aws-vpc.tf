...
/*
  ...
  - static_routes_only = true so we explicitly add routes we want advertised
*/
...
# TGW route FROM private VPC TO on-prem
resource "aws_ec2_transit_gateway_route" "tgw_to_onprem" {
  destination_cidr_block         = "10.20.0.0/16" # On-prem private subnet
  transit_gateway_attachment_id  = aws_vpn_connection.tgw_to_strongswan.transit_gateway_attachment_id
