...
# Route table for the public subnet - only needs a route to the Internet
...
resource "aws_security_group" "vpc2_sg" {
  ...
  ingress {
    ...
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = [aws_subnet.vpc2_private.cidr_block]
...
resource "aws_instance" "strongswan" {
  ...
  # minimal userdata to install strongswan (will still need config!! See strongswan-config.txt)
  user_data = <<-EOF
          #!/bin/bash
          apt-get update -y
          DEBIAN_FRONTEND=noninteractive apt-get install -y strongswan iptables-persistent strongswan-swanctl
...
resource "aws_instance" "onprem_device" {
  ...
  # just installing some ping tools
  user_data = <<-EOF
          #!/bin/bash
          apt-get update -y
